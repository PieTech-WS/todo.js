import { Plugin, PluginConfig } from './pluginInterface';
    ...
    public loadPluginsFromDirectory(directory: string, platform: string): void {
        ...
        */
        ...
        if (!fs.existsSync(pluginsJsonPath)) {
            console.error(`未找到插件配置文件: ${pluginsJsonPath}`);
            return;
        }

        const pluginsConfig = JSON.parse(fs.readFileSync(pluginsJsonPath, 'utf-8')) as Array<string>;
